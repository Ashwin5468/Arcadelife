...
<a href="balance.php" class="active" >Balance edit</a>
</div>
</div>
  <!--Logo -->
...
<center>
<div>
<div class="container" style="width:725px; margin-top:230px;">
    <h1>Users list</h1>
    ...
    if(isset($_POST['tokens_submit'])) {
        // retrieve the new token count from the form
        $new_tokens = $_POST['tokens'];

        // retrieve the username of the user whose token count is being updated
        $username = $_POST['username'];

        // update the token count in the database
        $sql = "UPDATE userinfo SET Tokens='$new_tokens' WHERE username='$username'";
    ...
    if (is_object($result) && $result->num_rows > 0) {
        ...
        echo "<table><tr><th>S_No.</th><th>Username Name</th><th>Tokens</th><th>Edit Tokens</th></tr>";
        while($row = $result->fetch_assoc()) {
            // display the token count and a form to edit it
            echo "<tr><td>".$row["S_No."]."</td><td>".$row["username"]."</td><td>".$row["Tokens"]."</td><td>";
            echo "<form method='post'>";
            echo "<input type='hidden' name='username' value='".$row["username"]."'>";
            echo "<input type='number' name='tokens' value='".$row["Tokens"]."'>";
            echo "<button type='submit' name='tokens_submit'>Update</button>";
            echo "</form>";
            echo "</td></tr>";
        }
        echo"</table>";
